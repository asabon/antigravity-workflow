...
- **安全な作成（Draft 強制）**:
  ...
  - 文字化けやエスケープミスを防ぐため、長文になる場合は一時ファイルを生成し、`--body-file` で指定して実行します。
...
## 2. PR の更新（本文編集）
- **安全な編集**:
  - 本文を更新（Edit）する際、特に長文になる場合は一時ファイルを生成し、`gh pr edit <PR番号> --body-file <ファイル名>` で実行することを**強く推奨**します（文字化け・事故防止）。
